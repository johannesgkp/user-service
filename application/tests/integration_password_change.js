...
describe('REST API', () => {
  ...
  context('when trying to change the password of a user', () => {
    ...
    it('it should reply 400 in case one parameter(newPassword) is missing', () => {
      let opt = JSON.parse(JSON.stringify(options2));
      opt.url += userid + '/passwd';
      opt.headers['----jwt----'] = jwtHeader;
      opt.payload = {};
    ...
    it('it should reply 400 in case one parameter(oldPassword) is missing', () => {
      let opt = JSON.parse(JSON.stringify(options2));
      opt.url += userid + '/passwd';
      opt.headers['----jwt----'] = jwtHeader;
      opt.payload = {};
    ...
    it('it should reply 403 in case the old password is wrong', () => {
      ...
      opt.payload = {};
      opt.payload.oldPassword = fullData.password;
	  //isnt this^ the right password?
      //opt.payload.oldPassword = 'wrongPassword';
      opt.payload.newPassword = 'abcdefgh';
      return server.inject(opt).then((response) => {
        response.should.be.an('object').and.contain.keys('statusCode', 'payload');
